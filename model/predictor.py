...
def generate_predictor_reasoning(model, tokenizer, input_zip, output_ans, args):
    """
    Query the Predictor model to ask for the reasoning behind its chosen answer.
    
    Args:
        model: The predictor model object (or model name if API).
        tokenizer: The predictor tokenizer (or None if API).
        input_zip: List of context/question strings.
        output_ans: List of previously predicted answers by the predictor.
        args: Command-line arguments.
        
    Returns:
        A list of reasoning strings generated by the predictor.
    """
    temperature_cot = 0.7
    ans_llm = []

    if getattr(args, 'data', '') == "xcopa_vi":
        instruction = "Dưới đây là một câu hỏi và câu trả lời bạn đã chọn. Hãy giải thích lý do tại sao bạn chọn câu trả lời này. Tuyệt đối không sinh ra các bước suy luận dài dòng gián tiếp, không được dùng các thẻ như <think>, chỉ ghi ngắn gọn lời giải thích của bạn bên trong thẻ [explain] và kết thúc bằng dấu @ (ví dụ: [explain]Lý do là...@), tuyệt đối không được lặp lại bất cứ cái gì khác."
        final_prompt = [ f"{instruction}\n\n### Câu hỏi: {ques}\n\n### Câu trả lời của bạn: {ans}\n\n### Giải thích của bạn:" for ques, ans in zip(input_zip, output_ans) ]
    else:
        instruction = "Below is a question and the answer you selected. Please explain why you chose this answer. Do not output long step-by-step reasoning, do not use any tag like the <think> tags, only write down the explanation enclosed in [explain] and end with @ (e.g., [explain]The reason is...@), absolutely do not repeat anything."
        final_prompt = [ f"{instruction}\n\n### Question: {ques}\n\n### Your answer: {ans}\n\n### Your explanation:" for ques, ans in zip(input_zip, output_ans) ]
        
    if args.pred_model in ["phi", "qwen"] and tokenizer is not None:
        tokenizer.padding_side = "left"
        model_inputs_all = tokenizer(final_prompt, padding=True, truncation=True, max_length=2048, return_tensors="pt").to(model.device)
        generate_ids = model.generate(**model_inputs_all, **_gen_kwargs(temperature_cot), max_new_tokens=1000)
        ...
        for i in range(len(ans_tkn)):
            split_str = "### Giải thích của bạn:" if getattr(args, 'data', '') == "xcopa_vi" else "### Your explanation:"
            ans = ans_tkn[i].split(split_str)[-1].strip()
            
            start_idx = ans.find("[explain]")
            end_idx = ans.find("@", start_idx)
            if start_idx != -1 and end_idx > start_idx:
                ans = ans[start_idx+9:end_idx].strip()
            else:
                ans = ans.split("###")[0].strip() # Fallback
        ...
        for prompt in final_prompt:
            try:
                ...
                print(f"[ERROR] Predictor Reasoning API: {e}")
                ans = ""
        ...
        # Fallback for other models or if not implemented
        ans_llm = [""] * len(input_zip)
